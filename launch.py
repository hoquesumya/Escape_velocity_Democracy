...
def kill_overlays():
    ...
    try:
        ...
        print(
            bcolors.OKBLUE
            + "Killing any running overlay processes on VMs"
            + bcolors.RESET
        )
...
def load_files():
    ...
    try:
        ...
        print(
            bcolors.OKBLUE
            + "SCPing overlay, dvr.py, and topology.dat to VMs"
            + bcolors.RESET
        )
        for vm_ex_ip in vm_ex_ips:
            ssh.connect(vm_ex_ip, username=uni, key_filename=key_filename)
...
def start_overlay(vm_name, vm_ex_ip, ready_queue):
    ...
    ssh.load_system_host_keys()
    ssh.connect(vm_ex_ip, username=uni, key_filename=key_filename)
    o_run_stdin, o_run_stdout, o_run_stderr = ssh.exec_command(
        rf"stdbuf -oL ./overlay {external_port} {internal_port} {topology_file} 2> \&1 &"
    )  # start overlay process
    time.sleep(
        2
    )  # wait a bit for overlay to start, then ensure that is started successfully, otherwise exit because won't ever work
    _, overlay_process_stoud, _ = ssh.exec_command("pidof overlay")
    if len(overlay_process_stoud.read()) == 0:
        print(
            bcolors.FAIL
            + f"VM  {vm_name} [Overlay]: Couldn't start the overlay on this VM. Please temrminate and rerun the program."
            + bcolors.RESET
        )
...
def start_peer(vm_name, vm_ex_ip):
    ...
    ssh.load_system_host_keys()
    ssh.connect(vm_ex_ip, username=uni, key_filename=key_filename)
    dvr_run_stdin, dvr_run_stdout, dvr_run_stderr = ssh.exec_command(
        f"stdbuf -oL python3 peer.py {external_port}"
    )
    ssh.close()
